...
def lambda_handler(data, _context):
    ...
    try:
        response = client.batch_update_findings(
            ...
            UserDefinedFields={
                'TicketOpen': data['ticket'].get('TicketOpen', 'Yes'),
                'TicketId': data['ticket'].get('TicketId', 'N/A'),
                'AccountDataJSON': json.dumps(data['account'])[:1024]
            },
            Note={
                'Text': f'Ticket created by {PRODUCT_NAME}',
...
def get_client(client_type, account_id, role=CROSS_ACCOUNT_ROLE):
    other_session = sts_client.assume_role(
        ...
        RoleSessionName=f"update_ticketed_finding_{account_id}"
    )
