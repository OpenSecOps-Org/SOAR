...
At the current release, the operative continuous-detection layer is the **release-time gate**: `./publish` runs `_check-requirements.sh` as its first step and refuses to release if the lock has drifted from `requirements.in`, if `pip-audit` flags any CVE not on the acknowledged-and-deferred list (§12), if any pinned artefact's SHA-256 fails verification against PyPI, or if any pinned package matches the OSSF malicious-packages feed.

Push-based and poll-based detection between releases — GitHub Dependabot alerts and a daily scan-only CI workflow on the public OpenSecOps repos — are planned for follow-up releases. When those land, this section gains the corresponding sentences and a brief description of how findings reach the maintainer.
