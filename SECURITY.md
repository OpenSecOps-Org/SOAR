...
A CycloneDX SBOM is attached as an asset to every GitHub release of this component on the public OpenSecOps remote:
...
The aggregate component-level SBOM is the union of every per-Lambda-function SBOM in the release, generated fresh at release time by `./publish`. Per-function SBOMs are derived artefacts (regenerated deterministically from `requirements.txt` on demand) and are not committed to the repository. (Sigstore signature/bundle artefacts will be added alongside in Phase 11.)
