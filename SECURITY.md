# Security Policy — SOAR

This document describes the security posture, governance model, vulnerability-reporting channel, and supply-chain assurances for the **SOAR** component of the OpenSecOps system. It is the single canonical reference for both supply-chain posture and governance posture; there is no separate `GOVERNANCE.md`.
...
A poll-based backstop — a daily scan-only CI workflow on each public OpenSecOps repository — is planned for a follow-up release; when it lands, a third bullet describes the workflow's permission model and findings-routing.

## 5. Supply-chain posture

This component was converted to the locked-dependency model in **v3.0.0**. The supply-chain posture below applies to v3.0.0 and every release after it. Earlier releases predate the conversion and do not carry these guarantees; per §1, the remediation for any exposure on an earlier release is to upgrade to the current release.
...
- No release-path CI; no auto-PR dependency bots; updates are deliberate maintainer actions.

### 5.1 Verifiability decomposition

The release artefacts (locks, per-function SBOMs, aggregate SBOM, evidence tarball, eventually signatures and provenance documents) give the customer four distinct *kinds* of independent verification, available at different points in the project's roadmap. Conflating them into a single "is this verifiable?" sentence would invite a careful reviewer to notice the elision; the four-way breakdown below states honestly which guarantees are independently verifiable today and which become so in follow-up releases.

1. **Independently verifiable today, no further release needed.** Two properties of the released artefacts can be reproduced with no maintainer cooperation:
   - **Hash integrity.** `pip download --require-hashes -r requirements.txt` against any published lock downloads each pinned wheel from PyPI and verifies its SHA-256 against the recorded hash. A mismatch indicates either tampering on the path PyPI → customer, or a stale lock; in either case the customer's `sam build` would also reject. Anyone can run this verification against the published `requirements.txt` files at any tag.
   - **Per-function SBOM determinism.** Re-running `compile-requirements.sh` against the same `requirements.txt` produces a byte-identical `requirements.cdx.json` (the UUIDv5 serialNumber is derived from `sha256(lock contents)` and the timestamp is pinned to a fixed epoch). The same property holds for the per-function `requirements.provenance.json` baselines. A reviewer who pulls the evidence tarball can regenerate every file in it and compare bit-for-bit.

2. **Documented today, independently verifiable in a follow-up release (substitution-in-transit).** Sigstore signatures on the SBOM, the evidence tarball, and the lock files will attest that the released bytes came from an authorised release identity recorded in the public Sigstore Rekor transparency log. This closes the question "did these bytes actually come from OpenSecOps, or did something between OpenSecOps and me substitute them?". Verification will be a one-line `cosign verify-blob` command; the exact invocation lands in §11 when signing ships.

3. **Documented today, independently verifiable in a follow-up release (gate-derivation, indirect closure).** A second machine recompiling from the committed `.in` files with the recorded compile-time timestamp passed as `--exclude-newer`, a clean `uv` cache, and the pinned `uv` version will produce bit-identical lock bytes. This is independent re-derivation of the gate's drift and hash-integrity outputs: if the lock is reproducible by anyone with the inputs, the gate's "is the lock well-formed and authentic" question is answered without trusting the maintainer's process. Lands when the `# uv-compiled-at:` timestamp header is added to every `requirements.txt`.

4. **Documented today, independently verifiable in a later follow-up release (gate-execution attestation, direct closure).** A SLSA Build L1 in-toto provenance document will declare the build steps that ran (`_check-requirements.sh`, `_aggregate-sbom.sh`, `_bundle-evidence.sh`, `cosign sign-blob`, etc.) for each release. This is the direct answer to "did `./publish`'s gate actually run, or did the maintainer skip it and sign anyway?". The Phase-11 reproducibility check from category 3 closes this question *indirectly* (if the lock is reproducible, the gate's outputs were correct regardless of process); the in-toto provenance closes it *directly* by naming the build steps in a signed attestation.

The categorisation is mechanism-attributed, not phase-bundled. Categories 2 and 3 are independent items inside the same follow-up release; category 4 closes once the in-toto provenance document ships.
...
> **Deferred to a follow-up release.** Release-artefact signing via Sigstore is planned. This section will then list the OIDC identities customers should accept when running `cosign verify-blob` against an OpenSecOps release artefact, alongside the exact verification command in §11.
...
> **Deferred to a future release.** A formal SLSA Build L1 provenance claim, with selected L2-adjacent controls (Sigstore signing; deterministic hash-pinned dependency resolution; reproducibility verification on a second machine) documented alongside, lands when an in-toto provenance document is generated by `./publish` per release. This section will then state the level *and* the L2-adjacent controls explicitly.

## 10. Software Bill of Materials (SBOM)

Two release assets are attached to every GitHub release of this component on the public OpenSecOps remote:
...
```
https://github.com/OpenSecOps-Org/SOAR/releases/download/<VERSION>/SOAR-<VERSION>-sbom.cdx.json
```

Replace `<VERSION>` with the release tag (e.g. `v3.0.0`). The asset is attached to every release on the public OpenSecOps-Org remote.
...
(Sigstore signature/bundle artefacts will be added alongside both assets in a follow-up release.)

## 11. Verifying release artefacts

> **Deferred to a follow-up release.** Release-artefact signing via Sigstore is planned. Once it ships, this section will contain the exact `cosign verify-blob` invocation customers can copy-paste, including the `--certificate-identity` and `--certificate-oidc-issuer` values that match the actual signing flow used.
