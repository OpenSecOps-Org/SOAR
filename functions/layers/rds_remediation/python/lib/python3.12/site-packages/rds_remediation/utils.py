...
def handle_not_found_error(error, resource_type, data):
    ...
    if (resource_type == 'instance' and error_code == 'DBInstanceNotFoundFault') or \
       (resource_type == 'cluster' and error_code == 'DBClusterNotFoundFault') or \
       (resource_type in ['snapshot', 'cluster_snapshot'] and error_code in 
        ['DBSnapshotNotFoundFault', 'DBClusterSnapshotNotFoundFault']):
